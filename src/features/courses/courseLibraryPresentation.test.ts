...
import { mostRecentIndependentPractice, publicLibraryCourses, redeemedUnlistedCourses } from "./courseLibraryPresentation";
...
describe("Course Library separation",()=>{
  ...
  it("keeps redeemed Unlisted Courses in their own library section",()=>expect(redeemedUnlistedCourses(courses).map(item=>item.id)).toEqual(["2"]));
