...
export default function LessonJourneyCard({ journey, recommended }: Props) {
  ...
  return <article aria-label={`${lesson.title}: ${journey.locked ? "Locked" : journey.current ? "Current" : learnerJourneyStateLabel(journey.state)}`} className={`rounded-2xl border p-5 shadow-sm sm:p-6 ${journey.locked ? "border-slate-200 bg-slate-100" : "bg-white"} ${recommended ? "border-blue-300 ring-2 ring-blue-100" : "border-slate-200"}`}>
    ...
    <p className="mt-4 text-sm font-semibold text-slate-700">{journey.locked ? "Locked — complete the previous lesson" : journey.current ? "Current" : journey.state === "empty" ? "No learning activities available yet" : learnerJourneyStateLabel(journey.state)}</p>
