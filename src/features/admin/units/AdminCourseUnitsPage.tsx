  ...
  return (
    <section className="mx-auto max-w-7xl">
      <PageHeader
        ...
        actions={<><ButtonLink icon="arrow-left" variant="secondary" to="/admin/courses">Back to courses</ButtonLink><ButtonLink variant="secondary" to={`/admin/preview/courses/${courseId}`}>Preview as Student</ButtonLink><ButtonLink variant="secondary" to={`?tab=curriculum`}>{activeTab === "curriculum" ? "Continue editing" : "Open curriculum"}</ButtonLink>{canPublish && course?.status !== "archived" && <Button type="button" isLoading={workspaceActionPending} onClick={() => void handleWorkspacePublish()}>Publish Course</Button>}{canEditDrafts && course?.status === "draft" && <Button type="button" variant="secondary" isLoading={workspaceActionPending} onClick={() => void handleWorkspaceDuplicate()}>Duplicate Course</Button>}{activeTab === "curriculum" && canEditDrafts && course?.status === "draft" && <Button icon="plus" onClick={() => { setFormErrorMessage(null); setFormState({ mode: "create" }); }}>Create unit</Button>}</>}
      />
