...
export function QuickActionsMenu({ label, actions }: { label: string; actions: QuickAction[] }) {
  ...
  return <div ref={rootRef} className="relative" onKeyDown={handleKeyDown}>
    ...
    {open && <div role="menu" aria-label={label} className="absolute right-0 z-20 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
      {actions.map((action) => <button key={action.label} role="menuitem" type="button" aria-disabled={action.disabled || undefined} title={action.explanation} onClick={() => { if (action.disabled) return; setOpen(false); action.onSelect(); }} className={`admin-focus flex min-h-11 w-full rounded-lg px-3 py-2 text-left text-sm font-semibold ${action.danger ? "text-red-700 hover:bg-red-50" : "text-slate-700 hover:bg-slate-50"} ${action.disabled ? "cursor-not-allowed text-slate-400" : ""}`}>
