...
import { canUploadMedia, canViewMediaLibrary, mediaLibraryUnavailableMessage, unavailableMediaLibraryService, type MediaAssetSummary, type MediaKind } from "../../../domain/media";
...
export default function AdminMediaLibraryPage() {
  ...
  const [state, setState] = useState<{ status: "loading" | "ready" | "unavailable" | "error"; items: MediaAssetSummary[] }>({ status: "loading", items: [] });
  ...
  useEffect(() => {
    if (!canView) return;
    let active = true;
    void unavailableMediaLibraryService.listMedia(query).then((result) => { if (active) setState({ status: "ready", items: result.items }); }, (error: unknown) => { if (active) setState({ status: error instanceof Error && "code" in error ? "unavailable" : "error", items: [] }); });
    return () => { active = false; };
  }, [canView, query]);
  ...
  return <section className="mx-auto max-w-7xl space-y-6">
    ...
    {state.status === "unavailable" && <div role="status" className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-950"><h2 className="font-bold">Media Library unavailable</h2><p className="mt-2 text-sm">{mediaLibraryUnavailableMessage}</p></div>}
    {state.status === "error" && <Alert tone="error">The Media Library could not be loaded. Refresh the page to try again.</Alert>}
    ...
    <Dialog isOpen={showUploadInfo} onClose={() => setShowUploadInfo(false)} title="Upload media" description="Shared-library management is not connected yet." footer={<Button type="button" onClick={() => setShowUploadInfo(false)}>Close</Button>}><p className="text-sm leading-6 text-slate-600">Upload media will activate after the pending backend integration. Existing direct uploads in Lesson Studio continue to work normally.</p></Dialog>
  </section>;
}
