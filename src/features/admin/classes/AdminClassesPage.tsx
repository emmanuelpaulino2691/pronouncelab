...
export default function AdminClassesPage() {
  ...
  return <section className="mx-auto max-w-7xl space-y-7">
    <PageHeader eyebrow="Teacher Workspace" title="My Classes" description="Manage your teaching groups." actions={canCreate ? <ButtonLink to="/admin/classes/new" icon="plus">Create Class</ButtonLink> : undefined} />
    {!canCreate && <p className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">Classroom management is available to administrators and teachers.</p>}
    ...
    {classes.length === 0 ? <EmptyClassesState /> : <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{classes.map((item) => <div key={item.id}>{item.name}</div>)}</div>}
  </section>;
}
