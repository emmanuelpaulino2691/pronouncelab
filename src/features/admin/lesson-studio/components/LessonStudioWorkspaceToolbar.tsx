...
export default function LessonStudioWorkspaceToolbar({ viewMode, onViewModeChange, sectionController }: { viewMode: StudioViewMode; onViewModeChange: (mode: StudioViewMode) => void; sectionController: ActivitySectionCollapseController | null }) {
  ...
  return <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm" aria-label="Lesson Studio workspace controls">
    ...
    <div className="flex gap-2" title={unavailable ? explanation : undefined}>
      <button type="button" disabled={unavailable} aria-label={`Collapse all sections${unavailable ? `. ${explanation}` : ""}`} onClick={() => { sectionController?.collapseAll(); setAnnouncement(`Collapsed ${sectionController?.sectionCount ?? 0} sections.`); }} className="min-h-10 rounded-lg border border-slate-300 px-3 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-50">Collapse All</button>
      <button type="button" disabled={unavailable} aria-label={`Expand all sections${unavailable ? `. ${explanation}` : ""}`} onClick={() => { sectionController?.expandAll(); setAnnouncement(`Expanded ${sectionController?.sectionCount ?? 0} sections.`); }} className="min-h-10 rounded-lg border border-slate-300 px-3 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-50">Expand All</button>
    </div>
