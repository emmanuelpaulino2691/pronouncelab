...
import { assignmentActionLabel, assignmentCourseStatus, assignmentStatusLabel } from "./assignmentPresentation";
...
export default function LearnerClassesPage() {
  ...
  return <MainLayout><section className="mx-auto max-w-5xl space-y-7">
    ...
    <div className="space-y-6">{items.map((membership) => <article id={`class-${membership.class_id}`} key={membership.class_id} className="scroll-mt-6 rounded-2xl border bg-white p-5 shadow-sm sm:p-6"><h2 className="text-xl font-bold">{membership.classes?.name}</h2>{membership.classes?.description && <p className="mt-2 text-sm leading-6 text-slate-600">{membership.classes.description}</p>}<h3 className="mt-6 text-sm font-bold uppercase tracking-wide text-slate-500">Assigned Courses</h3><div className="mt-3 grid gap-4 lg:grid-cols-2">{(assignments[membership.class_id] ?? []).map((assignment) => {
        ...
        return <section key={assignment.assignmentId} className="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h4 className="break-words text-lg font-bold">{assignment.courseTitle}</h4>{assignment.courseLevel && <p className="mt-1 text-sm text-slate-500">Level {assignment.courseLevel}</p>}</div><span className={`rounded-full px-3 py-1 text-xs font-bold ${status === "completed" ? "bg-emerald-100 text-emerald-800" : status === "in-progress" ? "bg-blue-100 text-blue-800" : "bg-slate-200 text-slate-700"}`}>{assignmentStatusLabel(status)}</span></div><p className="mt-4 text-sm font-semibold text-slate-800">{presentation.progressLabel}</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200" aria-label={`${assignment.navigation.percent}% complete`}><div className="h-full rounded-full bg-blue-600" style={{ width:`${assignment.navigation.percent}%` }} /></div><Link to={presentation.href} className="mt-5 inline-flex min-h-12 items-center justify-center rounded-xl bg-blue-600 px-5 py-3 text-center font-semibold text-white hover:bg-blue-700">{assignmentActionLabel(status)}</Link></section>;
