...
export default function UnitsPage() {
  ...
  return <MainLayout>
    <nav aria-label="Course navigation"><Link to="/courses" className="inline-flex min-h-11 items-center font-semibold text-blue-700 hover:underline">&larr; Course Library</Link></nav>
    <header className="mt-3 max-w-4xl"><p className="text-sm font-bold uppercase tracking-wide text-blue-700">Independent Practice</p><h1 className="mt-1 break-words text-3xl font-bold text-slate-950 sm:text-4xl">{course.title}</h1>{course.description.trim() && <p className="mt-3 text-lg leading-7 text-slate-600">{course.description}</p>}</header>
    {courseJourney.state === "empty" ? <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-7"><h2 className="text-2xl font-bold text-slate-950">This course does not have lessons available yet.</h2><p className="mt-2 text-slate-600">Browse another learning journey while new lessons are prepared.</p><Link to="/courses" className="mt-5 inline-flex min-h-11 items-center font-semibold text-blue-700 hover:underline">Browse other courses</Link></section> : <>
      <section aria-labelledby="course-progress-heading" className="mt-8 max-w-4xl"><div className="flex flex-wrap items-end justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Personal Progress</p><h2 id="course-progress-heading" className="mt-1 text-xl font-bold text-slate-950">{courseJourney.completedLessons} of {courseJourney.totalLessons} lessons completed</h2></div>{courseJourney.state === "completed" && <p className="font-semibold text-emerald-700">You completed every available lesson in this Course.</p>}</div><div className="mt-3"><ProgressBar value={courseJourney.percent} label={`${course.title} Personal Progress`} /></div></section>
