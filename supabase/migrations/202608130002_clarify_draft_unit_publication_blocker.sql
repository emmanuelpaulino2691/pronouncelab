...
-- Replace the complete publication transaction at the known 202608130001
-- boundary. Keeping this definition explicit avoids depending on the formatting
-- returned by pg_get_functiondef() on different database histories.
CREATE OR REPLACE FUNCTION "public"."publish_course"("requested_course_id" bigint) RETURNS "jsonb"
        ...
        loop
          ...
          if activity_row.type = 'theory' then
            ...
            if not theory_exists then
              errors := errors || jsonb_build_object('courseId', course_row.id, 'courseTitle', course_row.title, 'unitId', unit_row.id, 'unitTitle', unit_row.title, 'lessonId', lesson_row.id, 'lessonTitle', lesson_row.title, 'lessonVersionId', version_row.id, 'activityId', activity_row.id, 'activityTitle', activity_row.title, 'activityType', 'theory', 'category', 'activity', 'message', 'Learning content is required.');
            end if;
            ...
            if not found or listening_row.audio_asset_id is null then
              errors := errors || jsonb_build_object('courseId', course_row.id, 'courseTitle', course_row.title, 'unitId', unit_row.id, 'unitTitle', unit_row.title, 'lessonId', lesson_row.id, 'lessonTitle', lesson_row.title, 'lessonVersionId', version_row.id, 'activityId', activity_row.id, 'activityTitle', activity_row.title, 'activityType', 'listening', 'category', 'activity', 'message', 'Audio file is missing or has not been attached. Save the listening item.');
            else
            ...
            if not found or not public.pronunciation_entries_are_valid(pronunciation_row.block_type, pronunciation_row.entries, true) then
              errors := errors || jsonb_build_object('courseId', course_row.id, 'courseTitle', course_row.title, 'unitId', unit_row.id, 'unitTitle', unit_row.title, 'lessonId', lesson_row.id, 'lessonTitle', lesson_row.title, 'lessonVersionId', version_row.id, 'activityId', activity_row.id, 'activityTitle', activity_row.title, 'activityType', 'pronunciation', 'category', 'activity', 'message', 'Pronunciation content is incomplete.');
            end if;
            ...
            if not found or not public.is_valid_ai_speaking_mission_config(mission_row.config) then
              errors := errors || jsonb_build_object('courseId', course_row.id, 'courseTitle', course_row.title, 'unitId', unit_row.id, 'unitTitle', unit_row.title, 'lessonId', lesson_row.id, 'lessonTitle', lesson_row.title, 'lessonVersionId', version_row.id, 'activityId', activity_row.id, 'activityTitle', activity_row.title, 'activityType', 'ai_speaking_mission', 'category', 'activity', 'message', 'Mission instructions and prompt are required.');
            end if;
            ...
            if not found or not public.interactive_practice_is_complete(exercise_row.mode, exercise_row.config) then
              errors := errors || jsonb_build_object('courseId', course_row.id, 'courseTitle', course_row.title, 'unitId', unit_row.id, 'unitTitle', unit_row.title, 'lessonId', lesson_row.id, 'lessonTitle', lesson_row.title, 'lessonVersionId', version_row.id, 'activityId', activity_row.id, 'activityTitle', activity_row.title, 'activityType', 'interactive_practice', 'category', 'activity', 'message', 'Interactive Practice is incomplete.');
            end if;
